...
NSCN=`echo "scale = 2; $NSC / $AGE" | bc` # rescales to approx one (large) shift per lineage
...
# we would like to rescale to approximate the ClaDS things
NSCN0=`echo "scale = 2; $L0EST0 / ($NSH * $NSC)" | bc`
NSCN2=`echo "scale = 2; $L0EST2 / ($NSH * $NSC)" | bc`
...
echo ./runppl.sh $MODEL $TREE $RHO $LSH $LSC $MSH $MSC $NSH $NSCN $C1 $C2 $C3 $C4v $G1 $G2 $G3 $G4N false 2 1  $STEP $GUAN true $PART $ITER $NCORES 1 anadsGBM-LS2
